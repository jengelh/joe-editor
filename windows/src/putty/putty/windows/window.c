...
#ifdef JOEWIN
#define IDM_SCHEMES_MIN 0x1000
#define IDM_SCHEMES_MAX 0x5000
#define MENU_SCHEMES_STEP 16
#define MENU_SCHEMES_MAX ((IDM_SCHEMES_MAX-IDM_SCHEMES_MIN) / MENU_SCHEMES_STEP)
#else
...
#else
int PuttyWinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
#endif
{
    WNDCLASS wndclass;
    MSG msg;
...
#else // JoeWin-specific configuration loading
	...
	AppendMenu(cursor_menu, MF_ENABLED, IDM_CURSHORZ, "Underline");
	...
	for (j = 0; j < lenof(popup_menus); j++) {
	    ...
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
	    AppendMenu(m, MF_ENABLED, IDM_FONT, "");
    ...
    {
	POINT p;
	if (!jwLoadWindowLoc(&p))
	{
	    RECT cr;

	    if (GetWindowRect(hwnd, &cr))
	    {
		MoveWindow(hwnd, p.x, p.y, cr.right - cr.left, cr.bottom - cr.top, FALSE);
	    }
	}
    }
#endif
	...
	if ((unsigned)(n - WAIT_OBJECT_0) < (unsigned)nhandles) {
	    ...
	    sfree(handles);
#else
	pending_netevent = WAIT_OBJECT_0 == MsgWaitForMultipleObjects(1, &commQueueEvent, FALSE, INFINITE, QS_ALLINPUT);
#endif
...
static void update_cursor_check(void)
{
    CheckMenuItem(cursor_menu, IDM_CURSVERT, cfg.cursor_type == 2 ? MF_CHECKED : MF_UNCHECKED);
    CheckMenuItem(cursor_menu, IDM_CURSHORZ, cfg.cursor_type == 1 ? MF_CHECKED : MF_UNCHECKED);
    CheckMenuItem(cursor_menu, IDM_CURSBLOK, cfg.cursor_type == 0 ? MF_CHECKED : MF_UNCHECKED);
}

static void update_font_menu(void)
{
    char *boldstr;
    static char fontmenuname[64];
    int i;

    boldstr = cfg.font.isbold ? "bold " : "";
    if (cfg.font.height == 0)
	_snprintf(fontmenuname, 64, "Font: %s, %sdefault height", cfg.font.name, boldstr);
    else
	_snprintf(fontmenuname, 64, "Font: %s, %s%d-%s", cfg.font.name, boldstr,
		(cfg.font.height < 0 ? -cfg.font.height : cfg.font.height),
		(cfg.font.height < 0 ? "pixel" : "point"));
    
    for (i = 0; i < lenof(popup_menus); i++)
	ModifyMenu(popup_menus[i].menu, IDM_FONT, MF_BYCOMMAND | MF_ENABLED, IDM_FONT, fontmenuname);
}

void jwContextMenu(int hasblock)
{
    POINT cursorpos;
    MENUITEMINFO copyitem, pasteitem;

    copyitem.cbSize = sizeof(MENUITEMINFO);
    copyitem.fMask = MIIM_STATE;
    copyitem.fState = hasblock ? MFS_ENABLED : MFS_DISABLED;

    pasteitem.cbSize = sizeof(MENUITEMINFO);
    pasteitem.fMask = MIIM_STATE;
    pasteitem.fState = IsClipboardFormatAvailable(CF_UNICODETEXT) ? MFS_ENABLED : MFS_DISABLED;

    SetMenuItemInfo(popup_menus[CTXMENU].menu, IDM_COPY, 0, &copyitem);
    SetMenuItemInfo(popup_menus[CTXMENU].menu, IDM_PASTE, 0, &pasteitem);
...
static void update_mouse_pointer(void)
{
    ...
    switch (busy_status) {
      case BUSY_NOT:
#ifndef JOEWIN // We pretty much never want an arrow.
	if (send_raw_mouse)
	    curstype = IDC_ARROW;
	else
#endif
				...
				WPARAM wParam, LPARAM lParam)
{
	...
	switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
	  ...
	  case IDM_CURSHORZ:
	    if ((wParam & ~0xF) == IDM_CURSHORZ) { cfg.cursor_type = 1; }
	  case IDM_CURSBLOK:
	    if ((wParam & ~0xF) == IDM_CURSBLOK) { cfg.cursor_type = 0; }
	  ...
	  case IDM_FONT:
            {
		/* Taken from winctrls.c (~1928) */
		CHOOSEFONT cf;
		LOGFONT lf;
		HDC hdc;
		FontSpec fs = cfg.font;
...
#else
	    if (wParam >= IDM_SCHEMES_MIN && wParam < IDM_SCHEMES_MAX) {
		SendMessage(hwnd, WM_SYSCOMMAND, IDM_COLSCHEME, wParam);
	    }
#endif
...
#ifdef JOEWIN
// Be a little lenient with the cursor position -- don't pick up the previous character if we're barely over it
#define TO_CHR_X(x) ((((x)<0 ? ((x)-font_width+1) : ((x) + (font_width >> 1)))-offset_width) / font_width)
#else
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
      ...
      case WM_DROPFILES:
	{
		...
		/* Figure out mouse position at drop point */

		DragQueryPoint(hdrop, &mpos);

		x = min(term->cols, max(0, TO_CHR_X(mpos.x)));
		y = min(term->rows, max(0, TO_CHR_Y(mpos.y)));
		
		jwSendFiles(hdrop, x, y);
	       ...
	       unsigned long attr, int lattr)
{
...
#ifdef JOEWIN
	    i = 3;
	    starty = y + min(descent, font_height - 3);
#endif
	} else {
	    int xadjust = 0;
	...
	if (attr & TATTR_ACTCURS) {
		...
		MoveToEx(hdc, startx, starty, NULL);
		LineTo(hdc, startx + dx * length, starty + dy * length);
		if (!dx) { startx++; } else if (!dy) { starty++; }
	    }
#endif
	    oldpen = SelectObject(hdc, oldpen);
	    DeleteObject(oldpen);
...
{
	HDC hdc = ctx;
	RECT regn, updateRegn;

	regn.left = 0;
	regn.top = offset_height + topline * font_height;
	regn.right = 2 * offset_width + font_width * term->cols;
	regn.bottom = offset_height + (1 + botline) * font_height;

	//HideCaret(hwnd);
	ScrollDC(hdc, 0, font_height * -lines, &regn, &regn, NULL, &updateRegn);
	InvalidateRect(hwnd, &updateRegn, FALSE);
	//ShowCaret(hwnd);
}
