			    ...
			    void *resize_ctx)
{
    ...
    /* Ends up sending comm too early.  Don't really need it since we sneak this data across anyway */
#ifndef JOEWIN
...
static void toggle_mode(Terminal *term, int mode, int query, int state)
{
	...
	switch (mode) {
	  ...
	  case 1002:		       /* xterm mouse 2 (inc. button drags) */
	    ...
	    set_raw_mouse_mode(term->frontend, state);
	    break;
	  case 1005:		       /* xterm mouse 3 (extended mouse tracking) */
	  ...
	  case 1049:                   /* cursor & alternate screen */
	    ...
	    term->disptop = 0;
	    break;
	  case 2007:		       /* out-of-frame mouse tracking */
	    term->mouse_out_of_frame = 1;
	    break;
    } else
	switch (mode) {
		...
		Mouse_Action a, int x, int y, int shift, int ctrl, int alt)
{
	...
	int encstate = 0, r, c, maxxmit, maxcoord;
	char abuf[16];
	...
	if (term->ldisc) {
	    ...
	    maxxmit = (term->xterm_mouse == 3) ? 2047 : 255;
	    maxcoord = maxxmit - 33;

	    if (term->mouse_out_of_frame)
	    {
		maxcoord -= 15;
		r = (yorig < 0) ? (maxxmit + 1 + max(yorig, -15)) : (min(yorig, maxcoord) + 33);
		c = (xorig < 0) ? (maxxmit + 1 + max(xorig, -15)) : (min(xorig, maxcoord) + 33);
	    }
	    else
	    {
		r = min(y, maxcoord) + 33;
		...
		for (i = 0; i < 3; i++)
		{
			...
			abuf[n++] = (char)(0xc0 | (d >> 6));
			abuf[n++] = (char)(0x80 | (d & 0x3f));
		    }
		}

		ldisc_send(term->ldisc, abuf, n, 0);
	    }
	}
	return;
    }
